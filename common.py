...
def parse_ws_header(data):
    ...
    if "Sec-WebSocket-Key" not in header_dict:
        return None
    key_str = calculate_response_key(header_dict["Sec-WebSocket-Key"])
...
def parse_header(data):
    ...
    #头部32位为用户id
    aff = struct.unpack('>i', data[0:4])[0]
    if aff<1:
        logging.warn( "user:\t bad aff:"+str(aff) )
        return None
    #logging.warn(str(aff))
    data = data[4:]
    #用户流量超过限制等原因 被关闭
    try:
        jsonData = mc.get( str(aff) )
        user_data= json.loads(jsonData)
        if 'isClosed' in user_data:
            logging.warn( "user:\t"+str(aff) + ' out of transfer')
            return None
    except:
        pass
    addrtype = ord(data[0])
    ...
    if dest_addr is None:
        return None
    if dest_port >=10000:
        logging.info('forbidden connecting above 10000 %s:%d  aff:%d' % (to_str(to_bytes(dest_addr)), dest_port, aff))
        return None
    return addrtype, to_bytes(dest_addr), dest_port, (header_length+4),aff
